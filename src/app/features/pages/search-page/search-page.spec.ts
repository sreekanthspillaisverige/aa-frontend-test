...
describe('SearchPage', () => {
  ...
  it('remove history item removes it from history', async () => {
    component.removeHistoryItem('abc');
  ...
  it('does not render erro-state elements when errorMsg is null', async () => {
  ...
  it('render empty state when query is meaningful but no suggestions are returned', async () => {
    apiResultsSig.set([]);
    await typeQuery('abcd');
    expect(component.uiState()).toBe('ready');
    expect(component.suggestions()).toEqual([]);
